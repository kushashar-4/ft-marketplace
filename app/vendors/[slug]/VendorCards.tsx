...
export default function VendorCards({ vendor, menuItemsData }: VendorCardsProps) {
    ...
    const submitOrder = async () => {
        await supabase.from('orders').insert({vendor_id: vendor.id, total_price: totalPrice, client_email: userEmail})

        const { data: orders } = await supabase.from('orders').select().eq("vendor_id", vendor.id)
        await supabase.from('order_items').insert(selectedItems.map((item) => ({order_id: orders?.[orders.length - 1].id, item_name: item})));
